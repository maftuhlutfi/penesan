    ...
    return (
        <Link href={`/quiz/${slug}`}>
            <div className={`w-full group h-80 lg:h-96 relative rounded-2xl overflow-hidden shadow-quiz-card border-2 cursor-pointer border-gray-800 
                ...
                <h1 className='absolute bottom-0 w-full p-4 text-xl font-bold transition-all duration-300 ease-in transform bg-white lg:text-2xl lg:p-8'>{isIndo ? title.split(" || ")[0] : title.split(" || ")[1]}</h1>
                <div className='absolute flex gap-2 text-sm font-medium top-4 right-4'>
                    {categories.map((category, index) => <p className='p-2 bg-white border-2 border-gray-700 rounded-lg' key={index}>{isIndo ? category.title.split(" || ")[0] : category.title.split(" || ")[1]}</p>)}
                </div>
            </div>
        </Link>
    );
